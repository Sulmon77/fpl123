...
export function AdminSidebar({ adminPath }: AdminSidebarProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={cn('flex items-center gap-3 px-4 py-5 border-b border-white/10', collapsed ? 'justify-center' : '')}>
        ...
        {!collapsed && <span className="font-display font-bold text-white text-lg">FPL123 Admin</span>}
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? 'bg-brand-green text-brand-purple' : 'text-white/70 hover:text-white hover:bg-white/10',
      ...
      <div className="border-t border-white/10 p-3 space-y-1">
        <Link href="/" target="_blank" className={cn('flex items-center gap-3 px-3 py-2 rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-all text-xs', collapsed ? 'justify-center' : '')} title={collapsed ? 'View Site' : undefined}>
          <Zap className="w-4 h-4 flex-shrink-0" />
          {!collapsed && 'View Site'}
        </Link>
        <button
          onClick={async () => { await fetch('/api/admin/auth/logout', { method: 'POST' }); window.location.href = `/${adminPath}` }}
          className={cn('w-full flex items-center gap-3 px-3 py-2 rounded-lg text-white/50 hover:text-error hover:bg-error/10 transition-all text-xs', collapsed ? 'justify-center' : '')}
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className={cn('hidden lg:flex flex-col bg-brand-purple border-r border-white/10 transition-all duration-300 flex-shrink-0', collapsed ? 'w-16' : 'w-56')}>
        <SidebarContent />
        <button
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-brand-purple border-b border-white/10 px-4 h-14 flex items-center justify-between">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="text-white p-1.5 rounded-lg hover:bg-white/10">
