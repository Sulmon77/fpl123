...
## What's Already Done — Everything is Complete
...
- PayPal JS SDK button fully wired up in the entry flow
...
- PayPal order creation, capture, and browser button (fully integrated)
...
### Step 2.3 — Run the Additional Migrations

Three extra SQL migrations are needed to add columns that support the full feature set. Run each one separately in the SQL Editor (New query → paste → Run):

**Migration 1** — adds `gameweek_ended` to the settings table:
```sql
ALTER TABLE settings ADD COLUMN IF NOT EXISTS gameweek_ended boolean DEFAULT false;
```

**Migration 2** — adds `fpl_team_name` and `group_number` to the payouts table:
```sql
...
**Migration 3** — adds `total_entries` and `total_amount` to the giveaway_history table:
```sql
...
Leave the callback URLs as placeholders for now — you'll update them after deployment in Part 9.
...
`PAYPAL_CLIENT_ID` is used on the server. `NEXT_PUBLIC_PAYPAL_CLIENT_ID` is the **same value** — it's needed by the PayPal button in the browser. Both must be filled in with your PayPal Client ID.
...
Note: `NEXT_PUBLIC_PAYPAL_CLIENT_ID` is the same value as `PAYPAL_CLIENT_ID`. Both are required.
...
## PART 11 — Going Live with Real Payments
...
## PART 12 — Weekly Gameweek Operations
...
- [ ] If groups look wrong, use **Shuffle Groups** to randomly reallocate everyone
...
2. [ ] **GW Controls** → click **End Gameweek** → confirm in the modal
   - This closes registration and unlocks the Payouts section
3. [ ] **Payouts** → **Generate Records** (creates a payout record for each winner)
4. [ ] **Payouts** → **Preview Payouts** → read through every winner and amount carefully
5. [ ] Tick the confirmation checkbox: "I have reviewed all payouts"
6. [ ] **Trigger All Payouts** → wait 1–2 minutes
7. [ ] Confirm all payouts show **sent** status
8. [ ] **Update Hall of Fame** (button appears after payouts are sent)
9. [ ] **Announce Results** (saves GW to History as hidden — confirm the browser prompt)
10. [ ] Optionally: **History** admin page → toggle the GW visible to public
11. [ ] **Announcements** → update the banner with results e.g. "GW22 Results: 🥇 John Doe (87 pts) wins KES 4,800!"
...
Check that `NEXT_PUBLIC_PAYPAL_CLIENT_ID` is set in both `.env.local` and Vercel environment variables. It must have that exact prefix — `NEXT_PUBLIC_` — to work in the browser.
...
Admin → Group Management → **Shuffle Groups** to randomly reshuffle everyone into new groups, or **Undo Allocation** to wipe all groups and start fresh. Entries and payments are never affected.

### Payouts section shows a lock screen
You need to end the gameweek first. Admin → GW Controls → click **End Gameweek**. This unlocks the Payouts section.
...
└── src/
    ...
    │   ├── page.tsx            ← Homepage
    │   ├── enter/
    │   │   └── page.tsx        ← Entry flow (FPL ID → verify → pay → PIN)
    │   │                          Includes live PayPal JS SDK button
    ...
    │       └── dashboard/      ← All admin sections
    │           ├── page.tsx    ← Dashboard home with stats
    │           ├── gw-controls/    ← Open/close GW, set fee, End Gameweek button
    │           ├── entries/        ← All payments, confirm/refund/disqualify
    │           ├── groups/         ← Allocate, shuffle, undo group allocation
    │           ├── standings/      ← Refresh points, view all groups
    │           ├── payouts/        ← Generate records, preview, trigger payouts,
    │           │                      update Hall of Fame, announce results
    ...
    ├── api/
    │   ├── settings/           ← Public settings for the frontend
    │   ├── fpl/verify/         ← Verify FPL ID and league membership
    │   ├── register/           ← Create entry before payment
    │   ├── mpesa/
    │   │   ├── initiate/       ← STK Push trigger
    │   │   ├── status/         ← Poll payment status
    │   │   ├── callback/       ← Safaricom webhook (confirms payment)
    │   │   └── b2c-result/     ← Safaricom B2C payout webhook
    │   ├── paypal/
    │   │   ├── create-order/   ← Creates PayPal order before button renders
    │   │   └── capture-order/  ← Captures payment after buyer approves
    │   ├── manager/[fplId]/    ← PIN auth for standings access
    │   ├── standings/[groupId]/← Returns group standings data
    │   └── admin/
    │       ├── settings/       ← Read and write all admin settings
    │       ├── stats/          ← Dashboard summary numbers
    │       ├── groups/         ← Fetch groups for current GW
    │       │   ├── undo/       ← Delete all groups for current GW
    │       │   └── shuffle/    ← Delete and reallocate groups randomly
    │       ├── assign-groups/  ← Trigger group allocation
    │       ├── end-gameweek/   ← Mark GW as ended, unlock payouts
    │       ├── payouts/
    │       │   ├── route.ts    ← Fetch payout records
    │       │   ├── preview/    ← Preview winners and amounts before paying
    │       │   └── generate/   ← Create payout DB records from standings
    │       ├── trigger-payouts/← Send M-Pesa/PayPal payments to winners
    │       ├── hall-of-fame/
    │       │   └── update/     ← Update all-time HOF after GW payouts
    │       ├── history/
    │       │   └── announce/   ← Write GW results to giveaway_history table
    │       ├── fetch-deadline/ ← Pull GW deadline from FPL API
    │       ├── entries/        ← Manage entries (confirm, refund, disqualify)
    │       ├── announcements/  ← Homepage banner CRUD
    │       ├── blacklist/      ← Manage blocked IDs/phones/emails
    │       └── cron/           ← Background job endpoints (deadline, groups, points)
    ├── components/
...
*Built with Next.js 15, Supabase, Tailwind CSS, M-Pesa Daraja API, PayPal REST API, and @paypal/react-paypal-js.*  
